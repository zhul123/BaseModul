...
//                Log.d("nestScrolling", "ACTION_MOVE:");
                //未完全显示
                if(!isGlobalVisible()){
...
//                Log.d("nestScrolling", "ACTION_SCROLL:");
                break;
...
//        Log.d("nestScrollingjianf", String.valueOf(rect.bottom - rect.top));
//        Log.d("nestScrollingheit", String.valueOf(viewHeight));
        //如果控件全部显示则拦截点击事件
        if (Math.abs(rect.bottom - rect.top - viewHeight) < 50) {
            return true;
        }
        return false;
    }
...
//        Log.d("nestScrolling", "nowY:"+nowY);
        ...
        if (visibleItemCount > 0) {
            if(visibleItemCount == totalItemCount){
                //如果已全部显示则交由父控件处理点击操作
                //如果上划和下滑都不能再滑动时，表示当前item元素完全展示，否则可能为部分展示一半情况
                if(!NestRecyclerView.this.canScrollVertically(1) || !NestRecyclerView.this.canScrollVertically(-1)) {
                    isTopToBottom = true;
//                    isBottomToTop = true;
                }
